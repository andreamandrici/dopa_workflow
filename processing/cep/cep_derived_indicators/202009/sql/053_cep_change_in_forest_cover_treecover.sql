-- SELECT THE THEME;
DROP TABLE IF EXISTS theme; CREATE TEMPORARY TABLE theme AS SELECT * FROM results_202009.r_univar_cep_gfc_treecover_over30;
-- SELECT THE GRID;
DROP TABLE IF EXISTS grid_index; CREATE TEMPORARY TABLE grid_index AS SELECT qid,eid FROM cep202009.z_grid ORDER BY qid,eid;
-- SELECT THE AREA;
DROP TABLE IF EXISTS area_index; CREATE TEMPORARY TABLE area_index AS SELECT eid,cid,(area_m2/1000000) sqkm FROM results_202009.cid_area_by_tile ORDER BY eid,cid;
------------------------------------------------------------
...
-------------------------------------------------------------
-- country
DROP TABLE IF EXISTS results_202009.country_change_in_forest_cover_treecover;
CREATE TABLE results_202009.country_change_in_forest_cover_treecover AS
SELECT
country country_id,
...
-- ecoregion
DROP TABLE IF EXISTS results_202009.ecoregion_change_in_forest_cover_treecover;
CREATE TABLE results_202009.ecoregion_change_in_forest_cover_treecover AS
SELECT
ecoregion eco_id,
sqkm gfc_treecover_km2,
prot_sqkm gfc_treecover_prot_km2,
...
-- pa
DROP TABLE IF EXISTS results_202009.wdpa_change_in_forest_cover_treecover;
CREATE TABLE results_202009.wdpa_change_in_forest_cover_treecover AS
SELECT
pa wdpaid,
sqkm gfc_treecover_km2,
perc gfc_treecover_perc

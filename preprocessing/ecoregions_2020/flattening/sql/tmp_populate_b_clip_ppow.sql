INSERT INTO eco2020_output.b_clip_ppow(tid,qid,fid,path,geom)
SELECT ROW_NUMBER () OVER () tid,* FROM 
(SELECT qid,fid,UNNEST(path) path,geom
FROM
(SELECT qid,fid,(ST_DUMP(geom)).* FROM
(SELECT qid,UNNEST(ppow) fid,geom FROM eco2020_input_ppow.h_flat ORDER BY qid,fid) a1
...
SELECT tid,(ST_ISVALIDDETAIL(geom)).* FROM eco2020_output.b_clip_ppow WHERE ST_ISVALID(geom);
DROP TABLE IF EXISTS geomz; CREATE TEMPORARY TABLE geomz AS
SELECT tid,ST_GEOMETRYTYPE(geom) FROM eco2020_output.b_clip_ppow WHERE ST_GEOMETRYTYPE(geom) !='ST_Polygon';

...
./a_input_land.sh ${ncores} > logs/a_input_land_log.txt 2>&1 wait
./a_input_marine.sh ${ncores} > logs/a_input_marine_log.txt 2>&1 wait
./a_input_abnj.sh ${ncores} > logs/a_input_abnj_log.txt 2>&1 wait
#./a_input_grid.sh ${ncores} > logs/a_input_grid_log.txt 2>&1 wait

# B-CLIP TABLES
## populates clip tables
./b_clip_land.sh ${ncores} > logs/b_clip_land_log.txt 2>&1 wait
./b_clip_marine.sh ${ncores} > logs/b_clip_marine_log.txt 2>&1 wait
./b_clip_abnj.sh ${ncores} > logs/b_clip_abnj_log.txt 2>&1 wait
#./b_clip_grid.sh ${ncores} > logs/b_clip_grid_log.txt 2>&1 wait


# C-RASTER TABLES
## populates raster tables
./c_rast_land.sh ${ncores} > logs/c_rast_land_log.txt 2>&1 wait
./c_rast_marine.sh ${ncores} > logs/c_rast_marine_log.txt 2>&1 wait
./c_rast_abnj.sh ${ncores} > logs/c_rast_abnj_log.txt 2>&1 wait
#./c_rast_grid.sh ${ncores} > logs/c_rast_grid_log.txt 2>&1 wait

# D-TILED TABLES
## DA-populates tiled tables (in parallel)
./da_tiled_land.sh ${ncores} > logs/da_tiled_land_log.txt 2>&1 wait
./da_tiled_marine.sh ${ncores} > logs/da_tiled_fmarine_log.txt 2>&1 wait
./da_tiled_abnj.sh ${ncores} > logs/da_tiled_abnj_log.txt 2>&1 wait
#./da_tiled_grid.sh ${ncores} > logs/da_tiled_abnj_grid.txt 2>&1 wait

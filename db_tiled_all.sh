...
## CLIP INPUTS
####################################################################################################################################################
REUNIONS_F="SELECT qid,geom FROM cep202003.da_tiled_country UNION ALL SELECT qid,geom FROM cep202003.da_tiled_ecoregion UNION ALL SELECT qid,geom FROM cep202003.da_tiled_wdpa"
...
SELECT qid FROM ${SCH}.z_1deg_grid
JOIN
...
psql ${dbpar2} -t -c "SELECT ${SCH}.f_pop_tiled_temp(${OBJ},'${SCH}','country');"
wait
psql ${dbpar2} -t -c "SELECT ${SCH}.f_pop_tiled_temp(${OBJ},'${SCH}','ecoregion');"
wait
psql ${dbpar2} -t -c "SELECT ${SCH}.f_pop_tiled_temp(${OBJ},'${SCH}','wdpa');"

...
### IMPORT AMPHIBIANS
psql ${dbpar2} -c "DROP TABLE IF EXISTS  ${SCH}.amphibians CASCADE;"
ogr2ogr \
-overwrite \
-dialect sqlite \
-sql "SELECT * FROM AMPHIBIANS WHERE presence IN (1,2) AND origin IN (1,2) AND seasonal IN (1,2,3)" \
-f "PostgreSQL"  PG:"$dbpar1" ${pgen}${prl}"/AMPHIBIANS/AMPHIBIANS.shp" \
-nlt "MULTIPOLYGON" \
-nln ${SCH}."amphibians"

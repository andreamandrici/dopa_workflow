...
2.  For aggregated results from above steps;
    1.  `db_tiled_all.sh` collects above geometries by tile in a single table; it starts the function `f_pop_tiled_temp()`, which will write results: 1) for each topic, on the table `db_tiled_temp`; 2) for all the topics, in the table `dc_tiled_all`;
